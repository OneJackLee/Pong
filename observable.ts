 ...
 *       IN Observable
 *
 *      filter to chain the condition instead of it else function..(we are able to convert our code from OOP to Functional programming)
 *      As we know that the reason of we use filter is to process data structure in some order 
 *      according to our defined predicate in order to produce a new data structure contain those same elements from original DS.
 * 
 *      takeWhile to stop evaluation after predicate requirement is violate(true-> false)
 *      In observable class I have added takeWhile funtion as takeWhile funtion works as mirroring 
 *      the Observable until predicate requirement becomes false,
 *      takeWhile will stop mirroring Observable and unsubscribe it
 * 
 *      subscribe to evaluate the instruction once been connected to Observable
 *      In observable class subscribe method is provided to subscribe an observer to this observable.
 *      As it will evaluate until unsubscribe requirement meet.
 *      
 */
...
class Observable<Input> {
  ...
  /**
   * creates a child Observable that is detached when the given function condition is meet
   * @param f function condition which will return true
   * @return child Observable
   */
  takeWhile(f:(_:Input) => boolean): Observable<Input> {
      return new Observable<Input>(observer =>
          this.subscribe(event=> f(event)?observer.next(event):undefined, () => {
