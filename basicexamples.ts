...
function drawRectsObservable() {
  ...
  const
    ...
    obj = Observable.fromEvent<MouseEvent>(svg,'mousedown')
            .map(e =>({
              rect: new Elem(svg, 'rect')
...
function drawAndDragRectsObservable() {
  ...
  const
  svg = document.getElementById("drawAndDragRects")!,             // get svg element
  mousemove = Observable.fromEvent<MouseEvent>(svg, 'mousemove'), 
  mouseup = Observable.fromEvent<MouseEvent>(svg, 'mouseup'),
  mousedown= Observable.fromEvent<MouseEvent>(svg, 'mousedown'),
  obj = Observable.fromEvent<MouseEvent>(svg,'mousedown')         // observe event mousedown
          .map(e =>({                                             // use the event to create a rectangle(temporary)
            rect: new Elem(svg, 'rect')
            .attr('x',e.clientX - svg.getBoundingClientRect().left)
            .attr('y',e.clientY - svg.getBoundingClientRect().top)
            .attr('width', '0')
            .attr('height', '0')
            .attr('fill', '#95B3D7')     
          })).map(({rect})=>                                     // map, to pass two more argument
          ({ 
            x : Number(rect.attr('x')),
            y : Number(rect.attr('y')),
            rect : rect,
          }))
          .flatMap(({x,y,rect})=>                               // flatMap to create multiple rect and selected the last one(mouseup)
            mousemove
            .takeUntil(mouseup)                                 // observe until mouseup
            .map(e=>                                            // return a new rectangle
              {
              return rect
                  .attr('x', String(Math.min(x, e.clientX - svg.getBoundingClientRect().left)))
                  .attr('y', String(Math.min(y, e.clientY - svg.getBoundingClientRect().top)))
            ...
            e.observe<MouseEvent>('mousedown')                // observe the mousedown event
            .map(function(newevent){                          // stopPropagation on event
                  newevent.stopPropagation()
                return newevent
            })
            .map(({clientX, clientY}) => ({ xOffset: Number(e.attr('x')) - clientX,             //map new coordinate
                                            yOffset: Number(e.attr('y')) - clientY }))
            .flatMap(({xOffset, yOffset}) =>
                mousemove
                  .takeUntil(mouseup)
                  .map(({clientX, clientY}) => ({ x: clientX + xOffset, y: clientY + yOffset })))
            .subscribe(({x, y}) =>                                                              // subscribe using curry function
            e.attr('x', x).attr('y', y))
          ).subscribe(s=>s)
      }
